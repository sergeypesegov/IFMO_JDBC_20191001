...
public class Task21 {
    ...
    //language=HSQLDB
    public String select03 = "SELECT * FROM EMPLOYEE WHERE salary >= 2000 AND salaty <= 3000";
    ...
    public String select08 = "SELECT DEPARTMENT.name AS depname, staff_size FROM DEPARTMENT LEFT JOIN (SELECT COUNT(*) AS staff_size, " +
            "DEPARTMENT FROM EMPLOYEE GROUP BY DEPRTMENT) AS amount ON amount.department = department.id  WHERE staff_size > 0";
